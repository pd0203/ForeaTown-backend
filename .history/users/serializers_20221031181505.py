...
class UserUpdateSerializer(WritableNestedModelSerializer): 
    class Meta: 
        model = User   
        fields = ('nickname', 'age', 'is_male', 'location')
    def validate(self, data): 
        if data['age'] < 19:
           raise ValueError('A ForeaTown user must be 19 years old or above')
        return data
    def update(self, instance, validated_data):
        # country_name = validated_data.pop('country')['name']
        # print("country name", country_name, " type", type(country_name))
        # print("country", Country.objects.get(name=country_name))
        instance.nickname = validated_data.get('nickname', instance.nickname)
        instance.age= validated_data.get('age', instance.age)
        instance.is_male = validated_data.get('is_male', instance.is_male)
        instance.location = validated_data.get('location', instance.location)
        # instance.country = Country.objects.filter(name=country_name)['id']
        instance.save()
        return instance
