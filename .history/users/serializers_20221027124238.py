...
class UserUpdateSerializer(WritableNestedModelSerializer): 
    ...
    def validate_status(self): 
        if self.status != '멘티' and self.status != '멘토':
    ...
    def validate_birthday(self):
        ...
        if birthday[3] == '1' or birthday[3] == '2' or birthday[3] == '5' or birthday[3] == '6':
           ...
           birthday[0] = '20' + birthday[0]
            ...
            raise ValueError('주민등록번호 7번째 자리 유효한 수는 0~9입니다')
        birthday.pop() 
        formatted_birthday = '-'.join(birthday)
        self.birthday = datetime.strptime(formatted_birthday, "%Y-%m-%d").date()
    def validate_is_male(self): 
        self.is_male = True 
        birthday = self.birthday.split('/')
        if birthday[3] == '1' or birthday[3] == '3' or birthday[3] == '5' or birthday[3] == '7' or birthday[3] == '9':
           self.is_male = True
        elif  birthday[3] == '0' or birthday[3] == '2' or birthday[3] == '4' or birthday[3] == '6' or birthday[3] == '8':
           self.is_male = False
        else: 
            raise ValueError('주민등록번호 7번째 자리에 유효한 수는 0~9입니다')
